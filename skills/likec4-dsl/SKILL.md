---
name: likec4-dsl
description: Use when working with `.c4`/`.likec4` files or LikeC4 CLI/config questions where exact DSL/CLI syntax is required, especially for strict command/snippet-first answers, validate/export flags, predicates `*`/`_`/`**`, deployment snippets, dynamic views, relationship extension matching, Draw.io export (`--profile leanix`), or LeanIX bridge workflows — load `references/bridge-leanix-drawio.md` when the task involves bridge artifacts, LeanIX sync, or bridge-managed Draw.io cells.
---
